...
locals {
  ...
  aws_arch = {"amd64": "x86_64"}[var.arch]
  runner_arch = {"amd64": "x64"}[var.arch]
}

source "amazon-ebs" "githubrunner" {
  ami_name                                  = "github-runner-lin-builder-${var.arch}-${formatdate("YYYYMMDDhhmm", timestamp())}"
...
build {
  ...
  provisioner "file" {
    content = templatefile("install-builder-runner.sh", {
      ARM_PATCH                       = ""
      S3_LOCATION_RUNNER_DISTRIBUTION = ""
      RUNNER_ARCHITECTURE             = local.runner_arch
    })
    destination = "/tmp/install-builder-runner.sh"
  }
